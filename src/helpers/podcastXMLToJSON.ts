...
export function podcastXMLToJSON (text: string, podcastId: string) {
  ...
  const description = channel.querySelector('description')?.textContent?.trim() as string
  ...
  const episodes = episodeElements.map((episodeElement) => {
    ...
    const description = episodeElement.querySelector('description')?.textContent?.trim() as string
